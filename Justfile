...
default_image := "ghcr.io/tulilirockz/debian-bootc:latest"
arch := arch()
...
initramfs:
    ...
    dracut --force --no-hostonly --reproducible --zstd --kver "$INSTALLED_KERNEL" --add "dmsquash-live dmsquash-live-autooverlay" /app/{{ workdir }}/initramfs.img |& grep -v -e "Operation not supported"'
    chroot "$CMD"
